...
def dataHtml(bs, db):
    ...
    for i in DaTa:
        Tradename = i.find('div', class_='p-name p-name-type-2').find('a', target='_blank').find(
            'em').get_text().strip()  # 获取到的商品名
...
def main(db):
    for i in range(1, 200, 2):  # 循环1到200每一个数输出一下
        ...
        url = "https://search.jd.com/Search?keyword=%E6%89%8B%E6%9C%BA&wq=%E6%89%8B%E6%9C%BA&page={}&s=57&click=0".format(
            i)  # 循环的书添加到链接形成要抓取的新链接
        bs = dataUrl(url)
        dataHtml(bs, db)
        time.sleep(3)
        print(url)
