...
def get_links(url):  ##创建参数
   header = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/84.0.4147.135 Safari/537.36'}  ##伪装头
   pagebody= urllib.request.Request(url,headers=header)  ##把头和身体连接
   page= urlopen(pagebody)
   bs = BeautifulSoup(page,'html.parser')
   return bs.find('div',{'id':'bodyContent'}).find_all('a',href=re.compile('^(https://bk.tw.lvfukeji.com/baike-)((?!:).)*$'))   ##返回
